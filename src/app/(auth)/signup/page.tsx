...
export default function SignupPage() {
  ...
  async function handleGoogleSignUp() {
    ...
    try {
      ...
      // Create user record + household org
      ...
      if (!signupResponse.ok) {
        ...
        if (data.error?.code === "USER_EXISTS") {
          ...
          if (sessionResponse.ok) {
            router.push(role === "learner" ? "/onboarding" : "/parent/dashboard");
            return;
          }
        }
  ...
  return (
    <div className="w-full max-w-sm">
      <div className="bg-white rounded-xl shadow-[0_2px_8px_rgba(26,25,23,0.08)] p-8">
        ...
        <div className="mb-6">
          <p className="mb-3 text-sm font-medium text-[#1A1917]">I am a...</p>
          <div className="grid grid-cols-2 gap-3">
            <button
              ...
              className={`rounded-lg border-2 px-4 py-3 text-sm font-medium transition-colors ${
                role === "learner"
                  ? "border-[#2D7A6E] bg-[#E4F0ED] text-[#2D7A6E]"
                  : "border-[#E5E0D6] bg-white text-[#5C5950] hover:border-[#949085]"
              }`}
            >
              Student
            </button>
        ...
        <p className="mt-6 text-center text-xs text-[#949085]">
          ...
          <a href="/login" className="text-[#2D7A6E] hover:underline">
            Sign in
          </a>
        </p>
      </div>
